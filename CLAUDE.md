...
- `lib/email.ts` — Email report logic + AI commentary (prompt, HTML/text templates, `generateCommentary` w/ 3-attempt retry across 5 validators, `runVerifierPass` second-Claude-call factual check)
- `lib/email-flow.ts` — **Single source of truth for sending the weekly email** (`runWeeklyEmail`). Idempotency, hard freshness gate, backfill, VK, AI commentary, send, audit-log. Both web preview and standalone scripts call this.
- `lib/email-alerts.ts` — `sendFailureAlert` to isaac@ on any failure path
- `lib/prices-refresh.ts` — IBKR + Polygon refresh + `refreshAllOpenPrices` orchestrator (IBKR primary, Polygon fallback w/ stale-price retry). Used by route handlers AND email-flow.
- `lib/vital-knowledge.ts` — VK fetcher (IMAP, Gmail). Filters to "Vital Talking Points Recap" subject only — 1 weekly recap, not 5 daily digests.
- `app/email/preview/page.tsx` — Email preview page (iframe preview, regenerate, send)
- `app/api/email/sends/route.ts` — Audit log API (used by Settings tab "Recent Email Activity" card)
- `scripts/run-weekly-email.ts` / `scripts/run-daily-refresh.ts` — Standalone Node entry points (no `localhost:3001` dependency). Manual: `npx tsx scripts/run-weekly-email.ts [--to=addr]`. Not yet wired into launchd (TCC blocks reading `~/Desktop/` from launchd; project would need to relocate or grant FDA to node).
- `lib/prices.ts` — Historical price backfill (IBKR primary, Polygon fallback)
- `app/api/prices/ibkr/route.ts` — Thin wrapper over `lib/prices-refresh.ts:refreshIbkrPrices`
- `lib/db.ts` — SQLite connection, schema, CRUD, trade table, audit log, blob→table migration, `email_sends` audit table
...
**Apr 24 (session 2)**: Weekly email hadn't fired automatically since Feb 13 — launchd was blocked by macOS TCC from executing scripts under `~/Desktop/` (stderr: `Operation not permitted`). The new daily-refresh job hit the same wall. Fix: moved all launchd-executed assets out of the TCC-protected Desktop. Scripts are copied from the repo to `~/Library/Application Support/stock-contest/scripts/` and logs land in `~/Library/Application Support/stock-contest/logs/`. Added `scripts/install-launchd.sh` to manage the copy/load/unload lifecycle for both jobs. Plists point at the non-protected paths. Proved end-to-end via `launchctl kickstart` — previously this produced a 96-byte stderr and zero-byte stdout; now it runs IBKR + backfill cleanly with zero stderr. `backup-db.sh` was left in the repo only (it reads `~/Desktop/stock-contest/data/contest.db`, which launchd also can't access); the call from `weekly-email.sh` was already non-fatal so a missing-script warning is harmless. Users still need to run `npm run refresh:install` / `email:install` after editing scripts.

**May 8**: Weekly-email reliability + content overhaul (`2f4e895`), triggered by audit finding only 2 of 13 expected emails had shipped (Feb 13, May 1) and the May 1 email had stale Thursday-as-Friday prices plus AI hallucinations (wrong attribution "both harvested INTC" when only Yitzi did, missed Yitzi's two HOOD adds, wrong count "trimmed LFMD twice on May 6" when it was once). Fixes layered:
- **Hard freshness gate** — `runWeeklyEmail` now calls `refreshAllOpenPrices` on every send and refuses with `StalePricesError` if priceDates don't match today after retries. Eliminates the Polygon-`/prev`-yesterday-as-today silent corruption (DB shape was right, content was stale).
- **VK filter** — `fetchVitalKnowledge` filters by subject `"Vital Talking Points Recap"` (the curated weekly recap email) instead of aggregating 5 daily digests. Prevents emphasis drift to whichever VK email was most recent.
- **Per-ticker prompt structure** — trade summary now `PLAYER → TICKER (count + intent header) → individual rows with pre-computed (Wed) day-of-week`. Plus a "MUST mention" checklist of (player, ticker) tuples. AI no longer derives counts, days, or attribution.
- **5-layer AI validation** with 3-attempt retry: `detectCommentaryViolations` (numbers + ranking-paraphrase regex), `detectFactualViolations.missedTrades` (paragraph-level coverage), `detectFactualViolations.unknownTickers`, and `runVerifierPass` (second Claude call grading prose against trade log; outputs `ERROR: <TYPE> | <snippet> | <truth>` lines or `NO_ERRORS_FOUND`). Verifier catches counting + day-grouping errors that regex misses.
- **Architectural decoupling** — extracted `lib/prices-refresh.ts` (IBKR + Polygon + orchestrator) and `lib/email-flow.ts` (`runWeeklyEmail`) from route handlers. Routes are thin wrappers; preview and send share one code path. New `scripts/run-{weekly-email,daily-refresh}.ts` standalone Node scripts that don't need `localhost:3001` (manual invocation works today; launchd integration still TCC-gated by `~/Desktop/` location).
- **Observability** — `email_sends` audit table records every attempt (status, recipients, violation counts, error). `lib/email-alerts.ts:sendFailureAlert` emails isaac@ on failure. New `/api/email/sends` + Settings-tab "Recent Email Activity" card.
- **Other fixes** — `install-launchd.sh` now copies all `*.sh` deps + verifies `launchctl load` (kills "Database backup failed" log line); iframe-race defense on preview Send; stale schedule comment. Live-verified end-to-end: 0 violations across all 5 layers, every claim in the prose matched the trade log, prices fresh. 326 tests (up from 298).
...
**Jun 12**: Three-phase reliability/stats/email overhaul (`867925a`, branch `reliability-stats-email`), executed via subagent-driven development (implementer + spec review + quality review per task). (1) **Reliability** — `priceHistory` writes now keyed by each bar's REAL session date on both server (`persistPrices`) and client (`applyPriceUpdate`), killing the prior-session-close-under-today's-key corruption at both layers; `etDateFromMs`/`addDays` UTC helpers (incl. fixing a pre-existing `getPriceStaleness` bug that only failed after 8 PM ET); pending-row send idempotency in `email_sends` (pending blocks 30 min; test sends record status `"test"` and never block); degraded-data operator alerts + reader-facing Data Notes email footer. (2) **Statistics** — `buildDailyValueSeries` extracted as the single series source (Sharpe refactored onto it); `getAdvancedStats` adds max drawdown w/ peak-trough dates, annualized volatility, Sortino (full-n downside deviation), Jensen's alpha/beta vs SPY (excess returns, common-dates-only, ≥20 obs), payoff ratio; surfaced on PlayerDetailCard. (3) **Email** — `contestStartDate` in `WeeklyReportData`; `detectMilestones` (leader change w/ week-1 guard, new high, ≥5% drawdown recovery) rendered as a deterministic banner with a number-free `aiText` variant for the prompt; per-player vs-SPY lines; 9-column Contest Statistics table; HTML-escaped Data Notes footer. Preview route now skips stale-price retry sleeps (`maxStaleRetries: 0`) and the preview page shows AI-validation residual counts. 384 tests (58 new).
...
- **launchd agents are blocked by macOS TCC from touching `~/Desktop/`, `~/Documents/`, `~/Downloads/`, etc.** Terminal.app has user-consented Full Disk Access so manual `bash scripts/foo.sh` works; launchd runs under a separate TCC context with no such grant. Symptom: stderr shows `Operation not permitted`, exit status 32256 (126 << 8), app-level log is empty. Fix: keep the project on Desktop for the user's convenience but copy launchd-executed scripts + their logs to `~/Library/Application Support/<project>/`. Plists must point at absolute paths in the non-protected location.
- **Watch `launchd-*-stderr.log`, not just the app's own log.** The job silently failing with `Operation not permitted` leaves the app-level log empty — looks like "the job never fired". The real signal is in stderr. Any time a launchd agent appears to not run, check `*-stderr.log` first.
...
- **AI gets calendar arithmetic wrong — pre-compute day-of-week.** Passing dates as `2026-05-06` lets the AI name days itself, and it does so badly: claimed 5/6/2026 was a Tuesday when it's a Wednesday. Pre-compute and inject inline: `2026-05-06 (Wed)`. Same lesson as "pre-compute everything for the AI" applied specifically to calendar math — date-to-DOW is *not* a free conversion in the model's head.

- **Layer regex validators + a verifier-pass second-Claude-call for factual accuracy.** Regex validators (`detectCommentaryViolations` for numbers/ranking phrases, `detectFactualViolations` for coverage + unknown tickers) are cheap and deterministic but cannot catch counting errors ("three tranches on May 6" when there were two), wrong day-grouping ("APP opened Friday" when it was Thursday), or attribution-by-inference ("both harvested INTC gains" when only one player did). For those, run a second Claude call with the prose + ground-truth trade log and ask it to flag `ERROR: <TYPE> | <snippet> | <truth>` lines. Wire into the existing N-attempt retry. ~$0.005/call. The verifier itself can be flaky — accept the prose if the verifier itself errors out, log the warning, don't block sending.

- **AI prompt: group trades by player → ticker → day with explicit count headers.** Flat per-trade lists make the AI conflate players ("Eli sold INTC" when only Yitzi did) AND miscount tranches. Per-ticker grouping with a header line summarizing intent (`LFMD (4 sells, full exit)`) and individual rows lets the AI just copy. Same lesson as "Pre-compute everything for the AI" but specifically for trade summarization.

- **Don't trust DB `currentPrices` at email-send time.** Polygon `/prev` writes the prior session's close under today's date if called pre-publication. The DB shape (priceHistory has `2026-05-08`) looks right; the content is yesterday's price. `runWeeklyEmail` now calls `refreshAllOpenPrices` before building report data and refuses with `StalePricesError` if priceDates don't match today. The freshness check must compare *barDate* to today, not just check that a row exists.

- **Key price writes by the bar's real date, on every layer that persists.** Refresh sources legitimately return prior-session closes (Polygon `/prev` pre-publication, IBKR off-hours). Write under `priceDates[ticker]`, falling back to today only when the bar date is unknown — and mirror the rule in client state (`applyPriceUpdate`), because the debounced PUT persists client `priceHistory` wholesale. Fixing only the server re-corrupts from the browser.

- **Audit rows that gate behavior need distinct statuses per intent.** `findBlockingWeeklySend` reads `ok` rows to block duplicate sends; test sends also recorded `ok` — two individually-correct pieces composing into "Friday dry-run silently suppresses the real email." Dry-runs now record status `"test"`. When an audit table becomes a control surface, every writer must declare intent, and only a whole-system review catches the composition bug.

- **Design validators and prompts together.** Any pre-computed text handed to the AI with "copy this verbatim" must not contain patterns the output validators reject. The drawdown milestone ("…after a 7% drawdown") would have tripped the numeric-violation regex on every faithful copy, burning all retries. Pattern: keep numbers in deterministic render paths; give the AI a number-free variant (`Milestone.aiText`).

- **A test suite that fails only at certain hours means local/UTC midnight mixing.** `getPriceStaleness` tests passed all day and failed after ~8 PM ET (fixture date derived via `toISOString()` = tomorrow's UTC date). If tests fail at night that passed in the afternoon, grep for UTC-derived dates compared against local midnights before suspecting the code under test.

- **Retry loops need a caller-controlled budget.** `refreshAllOpenPrices`'s 2×5-min stale retries are right for the scheduled send (prices will publish soon after close) and pure dead time for previews (off-hours, prices cannot become fresh). An optional `maxStaleRetries` lets each caller pick; the preview passes 0 and labels staleness instead of waiting for it to heal.
