...
- [x] Chain `/api/prices/backfill` into the dashboard "Refresh Prices" button — manual UI refresh now fills historical daily-bar gaps via IBKR (Polygon fallback) the same way the scheduled `run-daily-refresh.ts` script does. Eliminates the flat-line chart artifact that appears when the dashboard hasn't been refreshed in a while. `backfillPrices()` now returns the newly inserted per-ticker entries, which `DashboardTab.refreshPrices()` merges into local state so the chart re-renders without a page reload. End-to-end verified: filled 577 missing days across 31 tickers on first invocation. Memory `feedback_backfill_with_refresh.md` updated to cover the manual UI path. (2026-05-28, `c81208c`)
